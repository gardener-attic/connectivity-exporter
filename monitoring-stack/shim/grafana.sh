...
while true; do
  ...
  awk 'FILENAME == "cpuacct/cpuacct.usage"                {printf "cpu_usage_seconds_total{} %s\n", $1/1e9}
    ...
    memory/memory.stat               \
  | sed 's/{/{container="grafana",/' > /metrics/grafana.prom.$$
  mv /metrics/grafana.prom.$$ /metrics/grafana.prom.part
  wait
done &

while [ -z "$(ls /var/lib/grafana/dashboards)" ]; do
  sleep 1
  echo "Waiting for the grafana reloader"
done

su -s /bin/sh grafana -c "/run.sh $*" &
